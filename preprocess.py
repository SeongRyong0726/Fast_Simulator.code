...
class preprocess():
    ...
    def WS_preprocessing(self, matrix_1, matrix_2, mode):
        num_h2= matrix_2.shape[2]
        if (matrix_1.shape[1] != matrix_2.shape[1]):
            print("error : not same num_channel of two matrix")
        ####    padding on matrix1
        pad_matrix_1 = self.padding_matrix(num_h2, matrix_1)
        ####    Im2col
        Im2col_matrix_1, Im2col_matrix_2 = self.Im2col(pad_matrix_1, matrix_2, matrix_1.shape, mode) #.shape method사용하면 ( , , )나옴 [1]int 형으로 값쓸수 있다.

        return Im2col_matrix_1, Im2col_matrix_2


    def OS_preprocessing(self, matrix_1, matrix_2, mode, result_size):
    ...
    def preprocessing(self, matrix_1, matrix_2, mode, result_size):
